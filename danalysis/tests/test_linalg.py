...
def test_matrix_A_B_E(dm_example_72):
    ...
    A, B = linalg.matrix_A(dm_example_72, info), linalg.matrix_B(dm_example_72, info)
...
def test_row_removal_generator(dm_example_78):
    ...
    order = list(linalg.row_removal_generator(dm_example_78, info))
    assert order == [(0,), (1,), (2,)]
...
def test_ensure_nonsingular_A(dm_example_72, dm_example_78):
    info = meta.info(dm_example_72, [0.,0.,0.])
    del_row, col_order = linalg.ensure_nonsingular_A(dm_example_72, info)
    assert len(del_row) == 0
    assert_allclose(col_order,range(info.n_v))

    info = meta.info(dm_example_78, [0.,0.,0.])
    del_row, col_order = linalg.ensure_nonsingular_A(dm_example_78, info)
    ...
    del_row, col_order = linalg.ensure_nonsingular_A(dm, info)
    assert len(del_row) == 1
    assert del_row[0] == 2
    assert col_order in [
        [1,0,2],
    ...
    dm[0,2] = 1
    info = meta.info(dm, [0.,0.,0.])
    del_row, col_order = linalg.ensure_nonsingular_A(dm, info, remove_row_ids=[2], col_perm=[1,2,0])
    assert len(del_row) == 1
    assert del_row[0] == 2
    assert col_order == [1,2,0]

    with pytest.raises(ValueError):
        del_row, col_order = linalg.ensure_nonsingular_A(dm, info, remove_row_ids=[1], col_perm=[1,2,0])
    with pytest.raises(ValueError):
        del_row, col_order = linalg.ensure_nonsingular_A(dm, info, col_perm=[0,1,2])
    with pytest.raises(ValueError):
        del_row, col_order = linalg.ensure_nonsingular_A(dm, info, col_perm=[1,2,1])
