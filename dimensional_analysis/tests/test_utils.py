...
def test_tracked_matrix():
    ...
    assert_allclose(tm.indices[0], [1,4])
    assert_allclose(tm.indices[1], [0,1])
    tm.delete_rows(0)
    assert_allclose(tm.matrix, [[8,9]])
    print(tm.indices)
    assert_allclose(tm.indices[0], [4])
    assert_allclose(tm.indices[1], [0,1])
    ...
    assert_allclose(tm.indices[0], [1,3])
    assert_allclose(tm.indices[1], [0,4])
